...
if [ "$WITH_ALG_RGB" -eq 1 ]; then
  echo "==> Installing keyboard RGB driver (python3-pip + hid module)"
  if ! python3 -c "import hid" 2>/dev/null; then
    apt-get install -y python3-pip
    pip3 install --break-system-packages hid || pip3 install hid
  else
    echo "    hid module already installed — skipping"
  fi

  echo "==> Granting user access to the RGB device (udev rule + group)"
  RULE_FILE="/etc/udev/rules.d/60-alg-rgb.rules"
  if [ ! -f "$RULE_FILE" ]; then
    echo 'KERNEL=="alg_rgb", SUBSYSTEM=="misc", MODE="0660", GROUP="alg-rgb"' > "$RULE_FILE"
  fi
  groupadd -f alg-rgb
